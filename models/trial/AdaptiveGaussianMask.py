...
class FrequencyDomainMambaBlock(nn.Module):
    def __init__(self, channels, use_residual=True):
        ...
        # 自适应高斯掩码 - 传入通道数
        # self.mask_module = AdaptiveGaussianMask(
        #     channels=channels,
        #     auto_init=False,
        #     fully_adaptive=True,
        #     adaptation_rate=0.1  # 调整适应速率
        # )

        self.freq_module = AdaptiveGaussianMask(num_channels=channels,initial_sigma_x=30, initial_sigma_y=30)
    ...
    def forward(self, x):
        ...
        # all_freq_processed = self.all_freq_mamba(x)
...
class UNMambaBackbone(nn.Module):
    def __init__(self, in_channels=128, hidden_dim=64, out_dim=32, num_classes=10, use_residual=True,
...
if __name__ == '__main__':
    ...
    model = UNMambaBackbone(in_channels=285, num_classes=4).to(device)
